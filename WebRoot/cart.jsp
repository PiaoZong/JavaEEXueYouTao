...
<div id="doc">
	<div id="s_hdw">
		...
		<div id="s_tbar">
			<div class="s_hd">
				<div class="tbar_lft">您好，欢迎来到学友淘！<a href="login.jsp">请登录</a> | <a href="register.jsp">免费注册</a></div>
				<div class="tbar_rgt">
					<ul>
						<li class="first"><a href="my.jsp">我的订单</a></li>
						<li><a href="my.jsp">我的学友淘</a></li>
						<li><a href="#">购物车</a></li>
						<li><a href="#">收藏夹</a></li>
						<li><a href="/LOGO/admin/login.jsp">进入后台</a></li>
		...
		<div class="mmenu">
			<div class="s_hd">
				...
				<div id="s_cart">
					<ul>
						<li class="nums"><a href="cart.jsp" id="s_cart_nums1">购物车： <span><%=c.productcount() %></span>  件</a> <a href="cart.jsp" class="btn" id="s_cart_nums2"></a></li>
	...
	<div id="s_bdw">
		<div id="s_bd">
			...
			<div class="stepflow"><img src="images/step01.gif" width="980" height="32" alt="" /></div>
			<%if(productManager.getInstance().loadbyid(id).getId()!=0){%>
			<div class="addinfo">
				<strong><%=productManager.getInstance().loadbyid(id).getName() %> 已添加至购物车。</strong>
				<style type="text/css">
				.hand{cursor:pointer;}
				</style>
				<a onclick="window.location.href='index.jsp'" class="hand">返回继续购物</a>
			</div><!--addinfo end-->
			<%} %>
			<div class="cartlist">
				<table width="100%">
					<tr>
						<th>购物车中的商品</th><th>价格</th><th>返现</th><th>购买数量</th><th>发货时间</th><th>操作</th>
					</tr>
					<%cartitem item=null;
					...
					<tr bgcolor="#fffaf1">
						...
						<td><strong class="red">￥<%=item.getPrice() %></strong></td>
						<td>￥0.00</td>
						<td>
							<div class="addinput">
								<input type="text" name="qty_item_1" value="<%=item.getCount() %>" id="qty_item_<%=item.getProductid() %>" onKeyUp="setAmount.modify('#qty_item_1')" class="stext"/>
                        		<a class="add" onClick="setAmount.add('#qty_item_<%=item.getProductid() %>')" href="cart.jsp?id=<%=item.getProductid() %>&count=1"></a>
								<a class="reduce" onClick="setAmount.reduce('#qty_item_<%=item.getProductid() %>')" href="cart.jsp?id=<%=item.getProductid() %>&count=-1"></a>
							</div>
						</td>
						<td>有货，可当日出货</td>
						<td><a href="#" class="blue">移入收藏夹</a><br /><a href="deletecartpro.jsp?id=<%=item.getProductid() %>" class="blue">删除</a></td>
					</tr>
					<%} %>
					<tr>
						<td valign="top"><a href="deletecartpro.jsp?action=all"><img src="images/deleteicon.gif" /> 清空购物车</a></td>
						<td align="right" colspan="5">
							<p>共<%=c.productcount() %>件商品</p>
							<p style="margin-top:10px;font-size:14px;">总计金额(不含运费)：￥<%=c.gettotalprice() %>(商品金额) - ￥0.00(返现金额)= <strong style="font-size:18px;color:#d80000;">￥<%=c.gettotalprice() %></strong> </p>
						</td>
					</tr>
					<tr>
						<td style="border:none;padding-top:20px;" colspan="6"><input type="button" value="" id="" class="btnimg f-r" onclick="window.location.href='cart-information.jsp'" /><a class="f-r goonbtn" href="index.jsp"><img src="images/gooncat.gif" width="86" height="24" alt="" /></a></td>
					</tr>
				</table>
			</div><!--cartlist end-->
